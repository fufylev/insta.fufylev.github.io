...
import { loadFirstSetOfPictures, loadNextSetOfPictures } from '~/libs/api/API_gallery';
...
export const picturesUploadHandler = (lastVisible, user) => (dispatch) => {
    return new Promise((resolve, reject) => {
        dispatch(loadStart());
        if (lastVisible === null) {
            loadFirstSetOfPictures(user)
